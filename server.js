...
app.get('/api/lotto', async (req, res) => {
    try {
        const data = await getGameData('lotto');
        res.json(data);
    } catch (err) {
        res.status(502).json({ error: err.message || 'Failed to fetch Lotto data' });
    }
});

app.get('/api/euromillions', async (req, res) => {
    try {
        const data = await getGameData('euromillions');
        res.json(data);
    } catch (err) {
        res.status(502).json({ error: err.message || 'Failed to fetch EuroMillions data' });
    }
});
...
app.get('/api/debug/:game', async (req, res) => {
    ...
    try {
        const debug = await getGameDebug(game);
        res.json(debug);
    } catch (err) {
        res.status(502).json({ error: err.message || 'Failed to debug data' });
    }
});

app.listen(PORT, () => {
    console.log(`LuckyNumbers server running on http://localhost:${PORT}`);
});
...
async function getGameData(game) {
    ...
    const lastDraw = mainStats.lastDraw;
    ...
    cache.set(game, { timestamp: Date.now(), data: response });
    return response;
}

async function getGameDebug(game) {
    const statsUrl = await resolveStatsXlsxUrl(game);
    const workbook = await fetchXlsx(statsUrl);
    const sheets = workbook.SheetNames.map(name => ({
        name,
    ...
    for (const sheet of sheets) {
        ...
        const numberRows = rows
            ...
            .filter(({ row }) => Number.isInteger(row[0]) && row[0] >= 1 && row[0] <= config.mainRange);
        if (numberRows.length < config.mainRange - 1) continue;
        ...
        const gapCol = pickGapColumn(colStats, headerRow, numCol, timesCol, null, dateCol);

        const sampleRow = numberRows[0]?.row || [];
        debug.push({
            sheet: sheet.name,
            headerRow,
            numCol,
...
function pickTimesColumn(colStats, numCol, headerRow = []) {
    ...
    // Fallback: pick column with highest max that's NOT a date-like value
    // Excel date serials for recent years are > 40000, draw counts are typically < 5000
    let bestIdx = null;
    let bestMax = -Infinity;
    colStats.forEach((stat, idx) => {
        if (!stat || idx === numCol) return;
        // Skip columns that look like Excel date serials (> 30000)
